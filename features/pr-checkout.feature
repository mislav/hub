Feature: hub pr checkout <PULLREQ-NUMBER>
  ...
  Scenario: Same-repo
    ...
    And "git checkout -b fixes" should be run
    And "fixes" should merge "" from remote ""
    And "fixes" should push "" to remote ""

  Scenario: Checkout an editable pull request
    Given the GitHub API server:
      """
      get('/repos/mojombo/jekyll/pulls/77') {
        json :number => 77, :head => {
  ...
  Scenario: Checkout an editable pull request with custom name
    Given the GitHub API server:
      """
      get('/repos/mojombo/jekyll/pulls/77') {
        json :number => 77, :head => {
    ...
    Then "git fetch origin refs/pull/77/head:fixes-from-mislav" should be run
    And "git checkout fixes-from-mislav" should be run
    And "fixes-from-mislav" should merge "refs/heads/fixes" from remote "git@github.com:mislav/jekyll.git"
    And "fixes-from-mislav" should push "refs/heads/fixes" to remote "git@github.com:mislav/jekyll.git"
